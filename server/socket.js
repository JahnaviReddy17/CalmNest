...
export function setupSocket(io) {
  io.on("connection", (socket) => {
    ...
    socket.on("circle-message", async ({ topic, message, userId, name }) => {
      try {
        const userDisplay = name || userId.slice(0, 8) + "...";
        // Persist the anonymized display name, not the raw userId, so that
        // reloaded history matches what was shown live and doesn't leak a
        // logged-in user's real database ID to other Safe Circle members.
        const newMsg = await GroupMessage.create({ topic, userId: userDisplay, message });
