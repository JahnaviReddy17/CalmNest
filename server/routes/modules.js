...
router.put("/mentor/assign/:moduleId", authenticate, requireRole("mentor"), async (req, res) => {
  try {
    ...
    const newTargetUsers = userIds.filter(id => !targetModule.assignedUsers.some(u => u.toString() === id));
...
router.put("/user/complete/:progressId", authenticate, async (req, res) => {
  try {
    ...
    if (io && status) {
        // Mentors join their own personal room (named by their user id) via
        // the "join-user" socket event, same as every other personal
        // notification in this app — not the "mentor-<roomId>" chat room.
        io.to(progress.mentorId.toString()).emit("module-completed", {
           userId: req.user.id,
